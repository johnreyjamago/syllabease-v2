...
<body class="">
    <div class="opacity-80 p-4 pb-10 shadow bg-white border-dashed rounded-lg dark:border-gray-700 mt-14">
        <div>
            <h1 class="font-semibold text-xl flex justify-center pt-5">Program Educational Objectives</h1>
            <div>
                <div class="mb-10 pb-10">
                    @foreach ($poes as $poe)
                    <div class="ml-20 flex items-center tracking-wide leading-relaxed">
                        <p>{{ $poe->poe_code }} : {{ $poe->poe_description }}</p>
                        <form action="{{ route('chairperson.destroyPoe', $poe->poe_id) }}" method="Post">
                            @csrf
                            @method('DELETE')
    ...
    <div>
        <div class="ml-10 mt-2 pt-2 w-max hover:scale-105 transition ease-in-out bg-blue py-2 text-white rounded-lg hover:bg-blue">
            <form action="{{ route('chairperson.createPoe') }}" method="GET">
                @csrf
        ...
        <div class="ml-10 mt-2 pt-2 w-max hover:scale-105 transition ease-in-out bg-blue py-2 text-white rounded-lg hover:bg-blue">
            <form action="{{ route('chairperson.editPoe', $department_id) }}" method="GET">
                @csrf
                <button type="submit" class="flex  px-5   text-ml">
