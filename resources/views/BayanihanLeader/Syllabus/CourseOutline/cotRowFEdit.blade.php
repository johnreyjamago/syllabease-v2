...
<body>
  <div class="m-auto p-8 bg-slate-100 mt-[120px] p-2 shadow-lg bg-gradient-to-r from-[#FFF] to-[#dbeafe] rounded-lg w-11/12">
    {{-- <div class="max-w-md  w-[560px] p-6 px-8 rounded-lg shadow-lg"> --}}
        <img class="edit_user_img text-center w-[500px] m-auto mb-12" src="/assets/Re-Order Final Course Outline.png" alt="SyllabEase Logo">
      <table id="table" class="table w-full table-bordered">
        <thead>
          <tr>
            <th width="30px">#</th>
            <th>Alloted Time</th>
      ...
      <hr>
      <h5>Drag and Drop the table rows and <button class="bg-blue" onclick="window.location.reload()">REFRESH</button> the page to check the updated order.</h5>
    </div>
  </div>
    ...
    $(function() {
      ...
      function sendOrderToServer() {
        ...
        $.ajax({
          type: "POST",
          dataType: "json",
          url: "{{ route('bayanihanleader.updateCotRowF', ['syll_id' => $syll_id]) }}",
          data: {
            order: order,
            _token: token
          },
