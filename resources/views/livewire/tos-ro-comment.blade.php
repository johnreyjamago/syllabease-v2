    ...
    <div class="z-10 fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center">
        <div class="z-50 absolute bg-white bg-opacity-90 w-[320px] min-h-[250px] rounded shadow-lg font-sans">
            ...
            <div class="h-96 overflow-y-auto mb-[70px]">
                @foreach($tos_row_comments as $tos_r_comment)
                @php
                $bgColor = ($tos_r_comment->user_id == Auth::user()->id) ? 'bg-yellow' : 'bg-blue2';
                @endphp
                    ...
                    <div class=" rounded flex justify-between">
                        <div class="flex justify-row">
                            <div class="w-10 h-10 p-1 border border-gray3 {{ $bgColor }} flex justify-center items-center m-1">
                                <div class="">{{ strtoupper(substr($tos_r_comment->firstname, 0, 1)) . strtoupper(substr($tos_r_comment->lastname, 0, 1)) }}</div>
                            </div>
                            <div class="ml-2">
                                <div class="font-semibold">
                                    {{$tos_r_comment->firstname . ' ' . $tos_r_comment->lastname}}
                                </div>
                                <div class="text-gray3">
                                    <p class="text-xs">{{ $tos_r_comment->tos_r_comment_created_at}}</p>
                                </div>
                                <div class="w-11/12 flex">
                                    {{$tos_r_comment->tos_r_comment_text}}
                                </div>
                            </div>
                        </div>
                        <div class="flex">
                            <form wire:submit.prevent="resolveComment('{{ $tos_r_comment->tos_r_comment_id }}')" method="POST">
                                <div class="tooltip hover:bg-green2 h-7 w-7 rounded-full flex justify-center items-center {{ $tos_r_comment->tos_r_comment_resolved_at ? 'bg-green2' : 'bg-white' }}">
                                    ...
                                    <span class="tooltiptext">
                                        @if($tos_r_comment->tos_r_comment_resolved_at)
                                        Mark as unresolved
                                        @else
                                        Mark as resolved
                                        @endif
            ...
            <div class="absolute border border-gray3 bottom-0 w-full rounded">
                <form wire:submit.prevent="addComment">
                    <div class="rounded flex justify-between bg-white">
                        <input type="hidden" wire:model="tos_r_id">
                        <input type="hidden" wire:model="col_no">
                        <input required wire:model="tos_r_comment_text" placeholder="Type your comment..." class="m-2 w-10/12 rounded h-[40px] p-2 placeholder:text-gray-400 focus:ring focus:outline-none">
                        <button type="submit">
