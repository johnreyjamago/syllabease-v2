<div class="">
    ...
    <div class="z-50 absolute right-0 top-0 bg-white bg-opacity-90 w-[320px] min-h-[250px]  rounded shadow-lg font-sans">
        ...
        <div class="h-96 overflow-y-auto mb-[55px]">
            @foreach($cot_f_comments as $cot_f_comment)
            @php
            $bgColor = ($cot_f_comment->user_id == Auth::user()->id) ? 'bg-yellow' : 'bg-blue2';
            @endphp
                ...
                <div class=" rounded flex justify-between">
                    <div class="flex justify-row">
                        <div class="w-10 h-10 p-1 border border-gray3 {{$bgColor}} flex justify-center items-center m-1">
                            <div class="">{{ strtoupper(substr($cot_f_comment->firstname, 0, 1)) . strtoupper(substr($cot_f_comment->lastname, 0, 1)) }}</div>
                        </div>
                        <div class="ml-2">
                            <div>
                                {{$cot_f_comment->firstname . ' ' . $cot_f_comment->lastname}}
                            </div>
                            <div class="text-gray3">
                                <p class="text-xs">{{ $cot_f_comment->cot_f_comment_created_at}}</p>
                            </div>
                            <div>
                                {{$cot_f_comment->cot_f_comment_text}}
                            </div>
                        </div>
                    </div>
                    <div class="flex">
                        <form wire:submit.prevent="resolveComment('{{ $cot_f_comment->cot_f_comment_id }}')" method="POST">
                            <input type="hidden" name="cot_f_comment_id" value="">
                            <div class="tooltip hover:bg-green2 h-7 w-7 rounded-full flex justify-center items-center {{ $cot_f_comment->cot_f_comment_resolved_at ? 'bg-green2' : 'bg-white' }}">
                                ...
                                <span class="tooltiptext">
                                    @if($cot_f_comment->cot_f_comment_resolved_at)
                                    Mark as unresolved
                                    @else
                                    Mark as resolved
                                    @endif
        ...
        <div class="absolute border border-gray3 bottom-0 w-full rounded">
            <form wire:submit.prevent="addComment">
                <div class="rounded flex justify-between bg-white">
                    ...
                    <input required wire:model="cot_f_comment_text" placeholder="Type your comment..." class="m-2 w-10/12 rounded h-[40px] p-2 placeholder:text-gray-400 focus:ring focus:outline-none">
                    <button type="submit">
